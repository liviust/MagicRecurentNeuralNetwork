...
def find_search_terms(card):
  ...
  if type != ["instant"] and type != ["enchantment"] and type != ["creature"]:
    search_words1.append(type)
  words = []
  for line in card[2:]:
    words += re.findall(r"\b(" + word_list + r")\b", line[:-1].lower().replace("creatures", "creature").replace("sacrifices", "sacrifice").replace("mana", "magic"))
  search_words2 = words #[]
  colors=[]
  if type == "artifact":
